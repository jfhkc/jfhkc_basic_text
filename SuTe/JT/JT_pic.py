...
def mysql_insert(conn, cur, car_shape_all):
    ...
    try:
        insert_sql_JT_Gears_dir = 'insert into zinnia.JT_Gears_dir(car_shape, car_brand, car_years, car_year_initial, jf_number, teeth, availableSizes) values(%s,%s,%s,%s,%s,%s,%s)'
        cur.executemany(insert_sql_JT_Gears_dir, car_shape_all)
    ...
    for jt_p in jt_3p[:2]:
        ...
        for jf_shape in jt_shapes:
            try:
                ...
                url_part=(jf_shape.xpath(".//a[2]/img/@src")[0])
                # print(url_part)
                pic_url='https://www.jtsprockets.com/'+url_part.replace(" ","%20")
                print(f"链轮编号: {jf_number}")
                # print(f"图片url: {pic_url}")
                pic_res=requests.get(pic_url,headers=headers,timeout=10, stream=True, verify=False)
                pic_content=pic_res.content
                with open(f"./JT_pics/{jf_number}.jpg", "wb") as fp:
...
if __name__ == '__main__':
    ...
    if not os.path.exists("./JT_pics"):
        os.mkdir("./JT_pics")
    for i in range(4001,6500):
        try:
